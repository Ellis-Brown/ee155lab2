...
//	c++ -std=c++11 -pthread -O2 server_breaker_styled.cxx ee155_utils.cxx server_breaker_thread.cxx

// We will always allocate a vector of this many lines, and then choose which
// ones to actually use on any run.
// Modified to print markdown formatted table of result by Ellis Brown, 2/22/22
...
void analyze_times_styled (vector<double> &times) {
...
static void run (int n_lines, int n_stores, int n_loads, int n_threads,
    ...
    /* The following for loop is added to test pick_unique_lines */
	...
	times.push_back (time);
        //cerr << "Execution took" << time << "ms";
	//LOG ("Execution took " << time << "ms");
    }
    analyze_times_styled (times);

    // Check that they stored the right data. 
    // Question: Why are we looking at the stored values. Is this to confirm they were correctly set
    if (n_stores > 0)
...
static void pick_unique_lines (vector<int> &indices, int n_lines) {
	...
	while (indices.size() < n_lines) {
	    int line = dist(gen); // note: random line in range (0, Max_Lines -1). Integer value.
